{ -------------------------------------------------------
        ...
        Короче, этот исходник написан "через жопу", в принципе как всегда :). Если кто его нормально проапгрейдит,
        то вышлите мне апгрейженый исходник =).
...
var
  ...
  SELECTEDOBJ : integer;                // текуще выбранный спец объект
...
implementation
uses unit2, teleport_dlg, Unit4, Unit5, Unit6, brshrepls;
{$R *.DFM}
...
if paramstr(1) <> '' then if fileexists(paramstr(1)) then Open037map(paramstr(1));
// loadmapp('"'+paramstr(1)+'"');
SELECTEDOBJ := -1;
GX := 32;GY := 32;
end;
...
with form1 do
        ...
        35 : label1.caption := 'Brush: respawn. red team';
        36 : label1.caption := 'Brush: respawn. blu team';
        ...
        43 : label1.caption := 'Brush: Soul Hunters Altar';                
...
if SHOWOBJ then
  for i := 0 to $FF do if ddd[i].active = true then
        ...
        for a := 0 to BRICK_Y-1 do begin
            if bbb[i,a].image > 0 then begin
            if bbb[i,a].image <= MAXBRUSH then
            ImageList.Items[0].Draw(DXDraw.Surface, GX+i*32,GY+a*16, bbb[i,a].image) else
            ImageList.Items[0].Draw(DXDraw.Surface, GX+i*32,GY+a*16, 0);
            end;
...
if SHOWOBJ then begin
  for i := 0 to $FF do if ddd[i].active = true then begin
                ...
                with form1.DXDraw.surface.canvas do begin
                        ...
                        if (ddd[i].objtype = 1) and (SELECTEDOBJ = i) then begin
                        Brush.Style := bsClear;
                        ...
                        if (ddd[i].objtype = 8) and (SELECTEDOBJ = i) then begin
                        Brush.Style := bsClear;
                        pen.color := rgb($FF,$FF,$0);
                        Pen.Style := psSolid;
                        MoveTo(GX+ddd[i].x*32+16, GY+ddd[i].y*16+8);
...
begin
  ...
  if extractfilepath(SaveDialog.initialdir) <> '' then SaveDialog.initialdir := extractfilepath(SaveDialog.filename);
...
if not (SaveDialog.execute) then exit;
  ...
  for a := 0 to BRICK_Y-1 do begin
          ...
          BlockWrite(F, buf, BRICK_X);
  end;
  for i := 0 to $FF do
        if ddd[i].active = true then
                blockwrite(f,ddd[i],sizeof(ddd[i]));
  CloseFile(F);
end;
...
begin
  ...
  for i := 0 to 255 do
        ddd[i].active := false;
  form1.SaveDialog.filename := extractfilename(filename);
  MAP_AUTHOR := header.Author;
  MAP_NAME := header.MapName ;
  MAP_BG := header.BG;
  BRICK_X := header.MapSizeX ;
  ...
  for a := 0 to header.numobj-1 do
       blockread(f,ddd[a],sizeof(ddd[a]));
  CloseFile(F);
end;
...
if (key=VK_F12) and (ShiftKeyDown) then begin
                ...
                for a := 0 to BRICK_y-1 do
                        if random(5) = 0 then
                        bbb[i,a].image := 36 else bbb[i,a].image := 0;
        end;
end;

procedure TForm1.About1Click(Sender: TObject);
begin
showmessage(form1.caption+#13+'created by   3d[Power]'+#13+'several upgrades by   [LD]:Surgeon'+#13#13+'http://powersite.narod.ru');
end;
