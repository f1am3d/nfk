...
type
  ...
  Q3Player = Object
    ...
    procedure singleupdate(time : glFLoat);
...
{---------------------------------------------------------}
procedure QuatFromMatrix(var Quat : Array of glFloat; m : TRotationMatrix);
var tr, s : glFloat;
    Q : Array[0..3] of glFloat;
    I, J, K : Integer;
...
begin
  ...
  begin
    ...
    quat[3] :=S/2;
    s :=0.5/s;
    quat[0] :=s*(m[2, 1] - m[1, 2]);
    quat[1] :=s*(m[0, 2] - m[2, 0]);
    quat[2] :=s*(m[1, 0] - m[0, 1]);
  end
  else
  begin         // diagonal is negative
    i := 0;
...
{--------------------------------------------------------------}
procedure InterpolateQuat(var fromM, toM : Array of glfloat; t : glFloat; var res : Array of glFloat);
var to1 : Array[0..3] of glFloat;
    acos, cosom, sinom, scale0, scale1 : glFloat;
begin
  // calc cosine
  cosom := fromM[0]*toM[0] + fromM[1]*toM[1] + fromM[2]*toM[2] + fromM[3]*toM[3];
  ...
  if (1.0 - cosom) > 0.1 then
  begin
    acos := arccos(cosom);
    sinom := sin(acos);
    scale0 := sin((1.0 - t) * acos) / sinom;
    scale1 := sin(t * acos) / sinom;
  end
  else
  begin
  ...
  res[0] := scale0*fromM[0] + scale1*to1[0];
  res[1] := scale0*fromM[1] + scale1*to1[1];
  res[2] := scale0*fromM[2] + scale1*to1[2];
  res[3] := scale0*fromM[3] + scale1*to1[3];
end;
...
{---------------------------------------------------------}
procedure MatrixFromQuat(var m : TRotationMatrix; quat : Array of glFloat);
var w, x, y, z : Array[0..2] of glFloat;
    x2, y2, z2 : glFloat;
begin
  x2 := quat[0] + quat[0];
  y2 := quat[1] + quat[1];
...
var I    : Integer;
    ...
    quat1, quat2, resQuat  : Array[0..3] of glFloat;
    Rotation, NextRotation : TRotationMatrix;
    Position, NextPosition : TVector;
    fm, iMatrix : TRotationMatrix;
begin
  Mdl.DrawModel;
  ...
  begin
    ...
    if pMdl <> nil then
    begin
      pol :=Mdl.FPS * (mdl.Poll);
      Position :=Mdl.tags[Mdl.frame * Mdl.Header.numTags + i].Position;
      Rotation :=Mdl.Tags[Mdl.Frame * Mdl.Header.numTags + i].Rotation;

      NextPosition :=Mdl.tags[Mdl.nextFrame * Mdl.Header.numTags + i].Position;
      NextRotation :=Mdl.Tags[Mdl.nextFrame * Mdl.Header.numTags + i].Rotation;

      QuatFromMatrix(quat1, Rotation );
      QuatFromMatrix(quat2, NextRotation );
      InterpolateQuat(quat1, quat2, pol, resQuat );
      MatrixFromQuat( fm, resQuat );
      imatrix := fm;
      ...
      m[10]:= imatrix[2, 2];
      m[11]:= 0;
      m[12] := position[0] + pol * (NextPosition[0] - position[0]);
      m[13] := position[1] + pol * (NextPosition[1] - position[1]);
      m[14] := position[2] + pol * (NextPosition[2] - position[2]);
      m[15] := 1;
...
{---------------------------------------------------------}
procedure Q3Player.singleupdate(time : glFLoat);
begin
{  Lower.UpdateFrame(time);
  if ANIMUP then Upper.UpdateFrame(time);
